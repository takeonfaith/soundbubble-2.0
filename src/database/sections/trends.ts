import { and, getDocs, limit, orderBy, query, where } from 'firebase/firestore';
...
export class Trends {
    ...
    static playlistPagination = new Pagination();
    ...
    static async getTrendingPlaylists(topQuantity: number) {
        try {
            this.playlistPagination.initialize();
            ...
            const q = query(
                ...
                orderBy('listens', 'desc'),
                limit(topQuantity),
                ...this.playlistPagination.constraints
            );

            const docs = await getDocs(q);

            this.playlistPagination.saveLastVisible(docs);
            const res = getDataFromDoc<TPlaylist>(docs);
