...
const loadPlaylistFx = createEffect(async (store: TStore) => {
	try {
		if (!store.loadingPlaylistId) throw new Error("Playlist id not specified")
		const playlist = await Database.Playlists.getPlaylistByUid(store.loadingPlaylistId)
...
sample({
	...
	source: $store,
	fn: (store, { playlist, songs }) => ({ ...store, loading: false, currentPlaylist: playlist, currentPlaylistSongs: songs }),
	target: $store,
})
